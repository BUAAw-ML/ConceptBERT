...
FIELDNAMES = ['image_id', 'image_h', 'image_w', 'unknown1','unknown2','unknown3','unknown4','num_boxes', 'boxes', 'features']
import csv
import base64
import pickle
...
def main(input_dir, output_dir):
    ...
    infiles.append(path + "/train2014_obj36.tsv")
    infiles.append(path + "/val2014_obj36.tsv")
    ...
    with env.begin(write=True) as txn:
        for infile in infiles:
            with open(infile) as tsv_in_file:
                ...
                for item in reader:
                    img_id = str(int(item["image_id"].split("_")[-1])).encode()
